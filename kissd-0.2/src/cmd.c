...
{
	char* curext = gExtensions;
	char* ext;
	char* ptr;

	/* if the list of allowed extensions is blank, anything goes */
	if (*curext == '\0')
		return 1;
	...
	while (1) {
		/* find the length of this extension */
		ptr = strchr (curext, ':');
		if (ptr == NULL)
			ptr = strchr (curext, '\0');

		if (!strncasecmp(ext, curext, (ptr - curext)))
			return 1;
...
cmd_list (struct THREAD_STATE* t, char* cmd)
{
  ...
  struct dirent* dent;
	...
	*ptr = 0;
	...
	if (basepath == NULL)
		/* empty list if the category is unknown or not configured */
		return 0;
	...
	while ((dent = readdir (dir)) != NULL) {
		...
		if (dent->d_type == DT_UNKNOWN) {
			/*
			 * POSIX says d_type does not have to be filled out (eg for NFS mounts).
			 * Therefore, we do a quick stat() and copy the information from there.
			 */
 ...
 *
 *  The connection must be closed on errors and short reads.
 */
static int
cmd_get (struct THREAD_STATE* t, char* cmd)
{
  ...
  char tmp_path[PATH_MAX];
	...
	*ptr++ = 0;
	...
	/* ensure the file requested resides in our allowed base paths */
	if (realpath (file, tmp_path) == NULL)
		return 1;
	if ((gAudioPath != NULL && !strncmp(tmp_path, gAudioPath, strlen (gAudioPath))) &&
	    (gVideoPath != NULL && !strncmp(tmp_path, gVideoPath, strlen (gVideoPath))) &&
	    (gPicturePath != NULL && !strncmp(tmp_path, gPicturePath, strlen (gPicturePath))))
		return 1;

	/* this is for extra paranoia: we do not allow the KiSS to accept any
	 * extension we do not allow */
	if (!verify_ext (file))
		return 1;
